...
// ---- Usage logging (Google Sheets via Apps Script) ----
// Paste the Web App URL from your Apps Script deployment here.
// Leave it blank ("") to disable logging with no errors.
const USAGE_LOG_URL = "https://script.google.com/macros/s/AKfycbxRZ9_ZibQMrvJE5LCFMtZ9WVxwVIMC3Pq_6KHj2UKLxrCGUfP_h4cYTtTZVFJgIr6c/exec";

function logUsageEvent(event, name, email) {
  if (!USAGE_LOG_URL) return;
  try {
    fetch(USAGE_LOG_URL, {
      method: "POST",
      mode: "no-cors", // Apps Script web apps don't return CORS headers
      body: JSON.stringify({ event, name: name || "", email: email || "" }),
    }).catch(() => {
      // Silently ignore — logging should never break the app for the user.
    });
  } catch {
    // ignore
  }
...
export function AuthProvider({ children }) {
  ...
  function signup(name, email, password) {
    ...
    saveUsers(users);
    setUser(key);
    logUsageEvent("signup", name.trim(), key);
    return true;
  }
  ...
  function login(email, password) {
    ...
    setUser(key);
    logUsageEvent("login", record.name, key);
    return true;
  }
